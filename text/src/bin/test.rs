...
fn main() -> Result<()> {
	...
	// Examples:
	// - matcher_for(GigaByte).amount(100.0, GigaByte).find_greater_than().exists()
	// - matcher_for(GigaByte).amount(100.0, GigaByte).label(label)

	println!("{:?}", parsed.match_for(GigaByte).amount((100.0, GigaByte)).is_greater_than().was_found());
